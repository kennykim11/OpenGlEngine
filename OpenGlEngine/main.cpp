...
int main()
{
	...
	glGenVertexArrays(1, &VAO); //creating vertex arrays
	glGenBuffers(1, &VBO); //creating vertex buffer
	glBindVertexArray(VAO); //activate the array
	glBindBuffer(GL_ARRAY_BUFFER, VBO); //sets the Array Buffer to VBO
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices), vertices, GL_STATIC_DRAW); //describes properies of the VBO
	//(index, numOfElementsInArray, typeOfElements, normalized aka [0, 1], sizeOfArray, offset??)
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3*sizeof(GLfloat), (GLvoid*)0); //defines an array of generic vertex attribute data for index 0
